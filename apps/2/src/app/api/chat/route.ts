...
const filterSupportedParts = (parts: unknown[]) =>
    parts
      .map(p => {
        const part = p as Record<string, unknown>
        if (part.type === 'text') return { text: part.text as string, type: 'text' as const }
        if (part.type === 'image') return { image: part.image as string, type: 'image' as const }
        if (part.type === 'file') return { file: part.file as string, name: part.name as string, type: 'file' as const }
        return null
      })
  ...
  POST = async (request: Request) => {
    ...
    if (!chat && message?.role === 'user') {
      await fetchMutation(api.chat.create, { title: 'New Chat' }, opts)
    ...
    if (message?.role === 'user' && !isToolApprovalFlow)
      await fetchMutation(
        api.message.create,
        { chatId, parts: filterSupportedParts(message.parts as unknown[]), role: 'user' },
        opts
      )
