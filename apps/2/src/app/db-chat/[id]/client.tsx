...
const getToolDisplayName = (toolName: string): string => {
  ...
  MessageItem = ({
    ...
    if (isUser)
      return (
        <div className='flex w-full items-start justify-end gap-3' data-role='user' data-testid='message'>
    ...
    return (
      <div className='flex w-full items-start justify-start gap-3' data-role='assistant' data-testid='message'>
        <SparklesAvatar />
  ...
  Client = ({ chatId, initialMessages }: ClientProps) => {
    ...
    useEffect(() => {
      if (query && !hasAppendedQuery) {
        ...
        window.history.replaceState({}, '', `/db-chat/${chatId}`)
      }
    }, [query, hasAppendedQuery, sendMessage, chatId])
    ...
    return (
      <div className='flex flex-1 flex-col overflow-hidden'>
        ...
        <div className='border-t p-4'>
          <ChatInput isBusy={isRunning} onAbort={handleAbort} onSubmit={handleSubmit} />
        </div>
      </div>
    )
  }
