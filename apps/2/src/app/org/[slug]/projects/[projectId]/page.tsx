...
import { enumToOptions } from '@a/cv/zod'
...
const priorityOptions = enumToOptions(orgScoped.task.shape.priority.unwrap()),
  PrioritySelect = ({ onValueChange, value }: { onValueChange: (v: Priority) => void; value: Priority }) => (
    <Select onValueChange={onValueChange as (v: string) => void} value={value}>
...
const TaskRow = ({ canAssign, canEdit, members, onAssign, onDelete, onToggle, onUpdate, task: t }: TaskRowProps) => {
    ...
    return (
      <div className='flex items-center gap-3 py-2'>
        ...
        {canAssign ? (
          <Select onValueChange={v => onAssign(v === 'none' ? null : (v as Id<'users'>))} value={assigneeId ?? 'none'}>
        ...
        <div className='flex items-center justify-between'>
          ...
          {canEditProject ? (
            <Button asChild variant='outline'>
              <Link href={`/org/${org.slug}/projects/${projectId}/edit`}>
