...
const MovieDetail = ({ movie }: { movie: Movie }) => (
    <div className='flex flex-col gap-4'>
      <div className='flex items-center gap-2'>
        <Badge variant={movie.cacheHit ? 'secondary' : 'default'}>
          {movie.cacheHit ? 'Cache Hit' : 'Cache Miss → Fetched'}
        </Badge>
        <span className='text-sm text-muted-foreground'>ID: {movie.tmdb_id}</span>
      </div>
      {movie.backdrop_path ? (
  ...
  Page = () => {
    ...
    return (
      <div className='mx-auto flex max-w-2xl flex-col gap-4 p-4'>
        <div className='flex items-center gap-2'>
          <Link className='rounded-lg p-1 hover:bg-muted' href='/movies'>
        ...
        <form
          ...
          <Input onChange={e => setId(e.target.value)} placeholder='TMDB ID (e.g. 27205)' value={id} />
        </form>
