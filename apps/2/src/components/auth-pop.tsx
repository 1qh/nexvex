...
const Logout = async ({ ...props }: ComponentProps<typeof PopoverPrimitive.Trigger>) => {
  const token = await tok(),
    { user } = api,
    { email, image, name } = token ? await fetchQuery(user.me, {}, { token }) : {}
  return (
    <Popover>
      <PopoverTrigger asChild {...props}>
        {token && image ? (
          ...
          <p className='size-8 shrink-0 rounded-full bg-muted-foreground' />
        )}
      </PopoverTrigger>
