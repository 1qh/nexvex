...
const PromptInputController = createContext<null | PromptInputControllerProps>(null),
...
export const LocalReferencedSourcesContext = createContext<null | ReferencedSourcesContext>(null)
...
export type PromptInputProps = Omit<HTMLAttributes<HTMLFormElement>, 'onError' | 'onSubmit'> & {
  ...
  onSubmit: (message: PromptInputMessage, event: SubmitEvent<HTMLFormElement>) => Promise<void> | void
...
export const PromptInput = ({
  ...
  const controller = useOptionalPromptInputController() as PromptInputControllerProps,
    ...
    [referencedSources, setReferencedSources] = useState<(SourceDocumentUIPart & { id: string })[]>([]),
  ...
  const openFileDialogLocal = useCallback(() => {
    ...
    addWithProviderValidation = useCallback(
      (fileList: File[] | FileList) => {
        const incoming = [...fileList],
          accepted = incoming.filter(f => matchesAccept(f))
        if (incoming.length && accepted.length === 0) {
        ...
        const currentCount = files.length,
          capacity = typeof maxFiles === 'number' ? Math.max(0, maxFiles - currentCount) : undefined,
          capped = typeof capacity === 'number' ? sized.slice(0, capacity) : sized
        if (typeof capacity === 'number' && sized.length > capacity)
          onError?.({
            code: 'max_files',
    ...
    clearReferencedSources = useCallback(() => setReferencedSources([]), []),
  ...
  const handleChange: ChangeEventHandler<HTMLInputElement> = event => {
    ...
    attachmentsCtx = useMemo<AttachmentsContext>(
      () => ({
        add,
    ...
    refsCtx = useMemo<ReferencedSourcesContext>(
      () => ({
    ...
    handleSubmit: SubmitEventHandler<HTMLFormElement> = event => {
        ...
        .then((convertedFiles: FileUIPart[]) => {
          try {
            ...
            if (result instanceof Promise)
              result
                // biome-ignore lint/nursery/noNestedPromises: x
                .then(() => {
                  clear()
    ...
    withReferencedSources = <LocalReferencedSourcesContext value={refsCtx}>{inner}</LocalReferencedSourcesContext>

  return <LocalAttachmentsContext value={attachmentsCtx}>{withReferencedSources}</LocalAttachmentsContext>
}
